#!/usr/bin/env python3
"""
25-Minute Validation Test - All NTP Correction Methods

Tests all NTP correction methods over 25 minutes with proper ground truth collection:
- none: No correction applied
- linear: Linear distribution of error
- drift_aware: Variance-weighted offset/drift attribution
- advanced: Temporal uncertainty degradation with variance weighting

Usage:
    python test_25min_all_methods.py none
    python test_25min_all_methods.py linear
    python test_25min_all_methods.py drift_aware
    python test_25min_all_methods.py advanced
"""
...
print("=" * 80)
print(f"25-MINUTE VALIDATION TEST - NTP Correction Method: {args.method.upper()}")
print("=" * 80)
...
# Update the correction method - FIXED PATH
# Real path: config['prediction_scheduling']['ntp_correction']['method']
...
# Initialize pipeline with method-specific config
...
# Wait for warmup timer to complete
time.sleep(5)
...
# Verify dataset state before starting
dataset_size = len(pipeline.dataset_manager.get_recent_measurements())
print(f"Dataset verification:")
print(f"  Total measurements in dataset: {dataset_size}")
print(f"  NTP correction method: {args.method}")
print(f"  NTP corrections processed: {pipeline.stats['ntp_corrections_applied']}")
print()
...
# CSV output with method-specific filename
timestamp_str = datetime.now().strftime('%Y%m%d_%H%M%S')
csv_filename = f"ntp_correction_{args.method}_test_{timestamp_str}.csv"
...
try:
    for i in range(iterations):
        ...
        # Track if NTP correction was applied
        if args.method != 'none' and 'ntp' in correction.source.lower():
...
# Calculate accuracy metrics if we have NTP ground truth
...
for source, count in sorted(sources.items()):
    ...
    print(f"  {source:25s}: {count:3d} times ({percentage:5.1f}%)")
print()
