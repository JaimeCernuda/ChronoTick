#!/usr/bin/env python3
"""
Create publication-quality plots for drift_aware 9-hour experiment results.
...
def load_data(csv_file):
    """Load the CSV file and filter for rows with NTP ground truth."""
    df = pd.read_csv(csv_file)

    # Filter to only rows where we have ground truth (ntp_offset_ms is not null)
    df_with_truth = df[df['ntp_offset_ms'].notna()].copy()
...
def plot_offset_comparison(df, output_path):
    ...
    system_error = df['system_error_vs_ntp_ms'].values  # Keep sign for offset
    chronotick_error = df['chronotick_error_vs_ntp_ms'].values
    chronotick_uncertainty = df['chronotick_uncertainty_ms'].values
...
def plot_composite_analysis(df, output_path):
    ...
    time_hours = df['elapsed_hours'].values
    system_error = df['system_error_vs_ntp_ms'].values
    chronotick_error = df['chronotick_error_vs_ntp_ms'].values
    chronotick_uncertainty = df['chronotick_uncertainty_ms'].values
    ...
    # Check if ChronoTick is better or worse
    ct_mean = df['chronotick_abs_error'].mean()
    sys_mean = df['system_abs_error'].mean()

    if ct_mean < sys_mean:
        improvement = (1 - ct_mean / sys_mean) * 100
        stats_text.append(f"\nImprovement:")
        stats_text.append(f"  Error Reduction: {improvement:.1f}%")
        stats_text.append(f"  Absolute Reduction: {sys_mean - ct_mean:.3f} ms")
    else:
        degradation = (ct_mean / sys_mean - 1) * 100
        stats_text.append(f"\nDegradation:")
        stats_text.append(f"  ChronoTick worse by: {degradation:.1f}%")
        stats_text.append(f"  Absolute increase: {ct_mean - sys_mean:.3f} ms")
        stats_text.append(f"\nNote: System clock was NTP-synced")
...
def main():
    """Main function."""
    # Load data
    data_dir = Path("homelab-execution-1/drift_aware_9hr_20251015")
    csv_file = data_dir / "chronotick_stability_20251015_152918.csv"

    print("Loading drift_aware test data...")
    ...
    composite_output = data_dir / "drift_aware_9hr_plots.png"
