#!/usr/bin/env bash
# System Clock Test - 2 Hours
# Pure system clock comparison (NO NTP, NO ChronoTick)
# Goal: Measure causality violations using ONLY raw system clocks over extended period

set -e

BASE_DIR="/mnt/common/jcernudagarcia/ChronoTick/data-streaming-evaluations"
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
TEST_NAME="system_clock_2hour_${TIMESTAMP}"
...
# Test parameters - 2 hours
NUM_EVENTS=3000
TEST_DURATION=7200  # 2 hours in seconds (120 minutes)
...
║   SYSTEM CLOCK TEST - 2 HOURS                             ║
║   Pure system clock comparison (NO NTP, NO ChronoTick)    ║
║   3000 events spread over 7200 seconds (~2.4s/event)      ║
╚════════════════════════════════════════════════════════════╝
EOF
echo -e "${NC}"
...
log_info "  Duration: $TEST_DURATION seconds (2 hours / 120 minutes)"
...
log_info "  - Extended 2-hour period for drift analysis"
echo
...
log_info "Expected completion: $(date -d "+7200 seconds" +"%Y-%m-%d %H:%M:%S")"
echo
...
log_info "Next: Analyze causality violations and clock drift over 2-hour period"
