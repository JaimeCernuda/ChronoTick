#!/usr/bin/env bash
# ChronoTick Test - 2 Minutes (Quick Validation)
...
TEST_NAME="chronotick_2min_${TIMESTAMP}"
...
# STEP 1: CLEANUP
log_info "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
log_info "STEP 1: CLEANUP"
log_info "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

log_info "Killing any existing processes..."
...
ssh $WORKER_B_NODE "cd $BASE_DIR && HF_HOME=/mnt/common/jcernudagarcia/.cache/huggingface TRANSFORMERS_CACHE=/mnt/common/jcernudagarcia/.cache/huggingface TRANSFORMERS_OFFLINE=1 HF_HUB_OFFLINE=1 PYTHONPATH=/mnt/common/jcernudagarcia/ChronoTick/server/src:$BASE_DIR /mnt/common/jcernudagarcia/ChronoTick/.venv/bin/python -m src.worker_chronotick \
    --node-id comp11 \
...
ssh $WORKER_C_NODE "cd $BASE_DIR && HF_HOME=/mnt/common/jcernudagarcia/.cache/huggingface TRANSFORMERS_CACHE=/mnt/common/jcernudagarcia/.cache/huggingface TRANSFORMERS_OFFLINE=1 HF_HUB_OFFLINE=1 PYTHONPATH=/mnt/common/jcernudagarcia/ChronoTick/server/src:$BASE_DIR /mnt/common/jcernudagarcia/ChronoTick/.venv/bin/python -m src.worker_chronotick \
    --node-id comp12 \
    --listen-port $WORKER_PORT \
    --ntp-server $NTP_SERVER \
    --chronotick-config $CHRONOTICK_CONFIG \
...
# Show sample ChronoTick data
if [ "$WORKER_B_EVENTS" -gt 0 ]; then
    log_info "Sample ChronoTick data from Worker B:"
    log_info "CSV Header:"
    head -1 "$RESULTS_DIR/worker_comp11.csv"
    echo
    log_info "Last event (showing ChronoTick fields):"
    tail -1 "$RESULTS_DIR/worker_comp11.csv" | cut -d, -f1-18
    echo
fi
...
if [ $COORD_EXIT -eq 0 ] && [ "$WORKER_B_EVENTS" -gt 0 ] && [ "$WORKER_C_EVENTS" -gt 0 ]; then
    ...
    echo
    log_info "Next: Deploy full 30-minute test with deploy_chronotick_30min.sh"
    exit 0
else
